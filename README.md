...
The table below presents the task descriptions used in the Basic CUDA Kernel Benchmark (which also serve as the basis for constructing the initial input prompts), along with the corresponding speedup results.
...
6 | Multi-Head Self-Attention | Implement a CUDA program for multi-head self-attention. Given three input matrices $Q$ (queries), $K$ (keys), and $V$ (values) of size $N \times d_{\text{model}}$, compute: $\text{MultiHead}(Q,K,V) = \text{Concat}(\text{head}_1,\ldots,\text{head}_h)$, where each head computes: $\text{head}_{i} = \text{softmax}\left(\frac{Q_{i} K_{i}^{T}}{\sqrt{d_{k}}}\right)V_{i}$ with $d_{k} = d_{\text{model}}/h$ and $Q_i$, $K_i$, $V_i$ being the $i$-th head's partition of the input matrices. | 0.6985 | 11.1264 |
